...
export type EscalationPathTarget =
  components["schemas"]["EscalationPathTargetV2"];
...
export const useEscalationPath = (
  escalationPathId: string | null,
  deps?: DependencyList,
) => {
  ...
  return useAsync(async () => {
    ...
    const response = await IncidentApi.request<
      components["schemas"]["EscalationsShowPathResultV2"]
    >({
    ...
    if (channelIds.length > 0) {
      const typesResponse = await IncidentApi.request<
        components["schemas"]["CatalogListTypesResultV3"]
      >({
        path: `/v3/catalog_types`,
      });
      const slackChannelType = typesResponse.catalog_types.find(
        (t) => t.type_name === "SlackChannel",
      );
      if (slackChannelType) {
        const entriesResponse = await IncidentApi.request<
          components["schemas"]["CatalogListEntriesResultV3"]
        >({
...
export const useSchedule = (
  ...
  return useAsync(async () => {
    ...
    const response = await IncidentApi.request<
      components["schemas"]["SchedulesShowResultV2"]
    >({
...
export const useOnCallData = (
  ...
  return useAsync(async () => {
    ...
    const typeResponse = await IncidentApi.request<
      components["schemas"]["CatalogShowTypeResultV3"]
    >({
      path: `/v3/catalog_types/${catalogTypeId}`,
    });

    const { schema } = typeResponse.catalog_type;
    const escalationAttr = schema.attributes.find(
      (a) => a.type === "EscalationPath",
    );
    ...
    if (!entry)
      throw new Error(
        `No incident.io catalog entry found for ${entityExternalId}`,
      );

    const showResponse = await IncidentApi.request<
      components["schemas"]["CatalogShowEntryResultV3"]
    >({
      path: `/v3/catalog_entries/${entry.id}?expand=true`,
    });
    const fullEntry = showResponse.catalog_entry;

    const escalationPath = escalationAttr
      ? (fullEntry.attribute_values[escalationAttr.id]?.value ?? null)
      : null;
    const schedule = scheduleAttr
      ? (fullEntry.attribute_values[scheduleAttr.id]?.value ?? null)
      : null;
...
export const useAllEscalationPaths = (deps?: DependencyList) => {
  ...
  return useAsync(async () => {
    const response = await IncidentApi.request<
      components["schemas"]["EscalationsListPathsResultV2"]
    >({
...
export const useAllSchedules = (deps?: DependencyList) => {
  ...
  return useAsync(async () => {
    const response = await IncidentApi.request<
      components["schemas"]["SchedulesListResultV2"]
    >({
      path: `/v2/schedules`,
    });
